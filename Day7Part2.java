...
//
        ...
        for (ArrayList<Integer> input : inputs) {
            operations.add(Utility.decimalToTertiary(0, input.size() - 1).toCharArray());
        }
            ...
            while (!allOperationsTested) {
                ...
                for (int j = 0; j < operationList.length; j++) {
                    switch (operationList[j]) {
                        ...
                        case '2' -> {
                            String concat = "" + output + input.get(j + 1);
                            output = Long.parseLong(concat);
                        }
                    }
                }
...
//                System.out.println("Target: " +solutions.get(i));
                if (output == solutions.get(i)) {
                    ...
                    operationValue++;
                    operationList = Utility.decimalToTertiary(operationValue, input.size() - 1).toCharArray();
                    if (operationValue >= Math.pow(3, operationList.length) || operationList.length >= input.size()) {
